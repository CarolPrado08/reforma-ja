...
import { ArrowLeft, ArrowRight, Calculator, User } from "lucide-react"
...
export function OrcamentoForm({ user, modo }: Props) {
  ...
  const STEPS = isProfissional
    ? ["Cliente", "Cômodo", "Serviço", "Metragem", "Região", "Revisão"]
    : ["Cômodo", "Serviço", "Metragem", "Região", "Revisão"]
  ...
  // índices lógicos das etapas técnicas (deslocados +1 se profissional)
  const offset = isProfissional ? 1 : 0
  const iComodo    = 0 + offset
  const iServico   = 1 + offset
  const iMetragem  = 2 + offset
  const iRegiao    = 3 + offset
  const iRevisao   = 4 + offset

  const onSubmit = async (data: OrcamentoInput) => {
    if (isProfissional && !data.nomeCliente?.trim()) {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <Card>
        <CardContent className="pt-6 space-y-6">
          ...
          {/* Step 0 (Profissional): Dados do cliente */}
          {isProfissional && step === 0 && (
            <div>
              <CardHeader className="px-0 pt-0">
                <CardTitle className="flex items-center gap-2">
                  <User className="w-5 h-5" style={{ color: "#1a5c2a" }} />
                  Dados do cliente
                </CardTitle>
              </CardHeader>
          ...
          {step === iComodo && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {COMODOS.map((c) => (
                  <button key={c} type="button" onClick={() => setValue("comodo", c)}
            ...
            <div>
              ...
              <div className="space-y-2">
                {TIPOS_SERVICO.map(({ label, key }) => (
                  <button key={key} type="button" onClick={() => setValue("tipoServico", key)}
          ...
          {step === iMetragem && (
            <div>
              ...
              <input
                type="number" min="1" max="10000" step="0.5" placeholder="Ex: 25"
                value={values.metragem || ""}
                onChange={(e) => setValue("metragem", parseFloat(e.target.value))}
                className="flex h-12 w-full rounded-md border border-input bg-background px-3 py-2 text-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
              <p className="text-gray-400 text-xs mt-2">Área total em metros quadrados</p>
              ...
              <div className="mt-4">
                ...
                <input type="range" min="0" max="0.5" step="0.05"
                  ...
                  className="w-full" style={{ accentColor: "#3B6D11" }}
                />
                <div className="flex justify-between text-xs text-gray-400"><span>0%</span><span>25%</span><span>50%</span></div>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {REGIOES.map(({ label, key }) => (
                  <button key={key} type="button" onClick={() => setValue("regiao", key)}
          ...
          {step === iRevisao && (
            <div>
              <CardHeader className="px-0 pt-0"><CardTitle>Revisão</CardTitle></CardHeader>
              ...
              {isProfissional && (
                <div className="mt-4 p-3 rounded-lg text-sm" style={{ background: "#eaf3de", border: "1px solid #b5d98a" }}>
                  ...
                  <p className="text-xs mt-1" style={{ color: "#27500a" }}>
                    Após criar, você receberá um link único para o cliente aprovar este orçamento.
                  </p>
                </div>
              )}
            </div>
